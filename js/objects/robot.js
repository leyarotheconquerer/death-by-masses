class Robot {
	...
	preload() {
		for (let sequence in this.images()) {
			console.log("loading", sequence);
			for (let index in this.images()[sequence])
			{
				let image = this.images()[sequence][index];
	...
	create(groups) {
		this.scene.anims.create({
			key: 'playerwalk',
			frames: this.images().walk.map(image => ({ key: image.name })),
			frameRate: 24,
			repeat: Phaser.FOREVER
		});
		this.scene.anims.create({
			key: 'playerattack',
			frames: this.images().attack.map(image => ({ key: image.name })),
			frameRate: 24,
			repeat: 0
		});
		...
		this.sprite = this.scene.physics.add
			...
			.play('playerwalk', false, Math.trunc(Math.random() * this.images().walk.length));
